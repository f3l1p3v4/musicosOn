...
export const userStore = create<UserStore>((set) => ({
  ...
  fetchUsers: async (token: string) => {
    try {
      const response = await fetch('http://31.97.26.156/api/users', {
        headers: {
  ...
  createUser: async (data: CreateUserInput, token: string) => {
    try {
      const response = await fetch('http://31.97.26.156/api/users/register', {
        method: 'POST',
        headers: {
  ...
  updateUser: async (data: UpdateUserInput, token: string) => {
    try {
      const response = await fetch(`http://31.97.26.156/api/users/${data.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
